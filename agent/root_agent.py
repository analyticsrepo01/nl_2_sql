...
def setup_environment(config_path: str = "config.yaml"):
    ...
    if not config_file.exists():
        raise FileNotFoundError(
            f"config.yaml not found. Searched in: {config_path}, "
...
def create_bigquery_toolset(config: dict):
    ...
    # Instantiate BigQuery toolset with forecast enabled
...
# MCP Toolbox disabled - using BigQuery forecast tool directly
# Uncomment below to enable MCP toolbox if needed:
# try:
#     toolbox = ToolboxSyncClient("http://127.0.0.1:5000")
#     mcp_tools = toolbox.load_toolset('bq-mcp-toolset')
#     print(f"✓ MCP Toolbox connected: {len(mcp_tools)} tools loaded")
# except Exception as e:
#     print(f"⚠️  MCP Toolbox not available: {e}")
#     mcp_tools = []
mcp_tools = []
